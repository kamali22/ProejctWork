...
import Category from "./buyerComponents/Category";
...
function Routing() {
  ...
  const handleChange = (item, d, setQuantity, quantity) => {
    ...
    axios.get("http://localhost:9000/cart").then((res) => {
      ...
      console.log("QUANTITY before change in handle change", quantity);
      setQuantity(item.quantity);
      console.log("QUANTITY change in handle change", quantity);
  ...
  console.log("CART DATA iin", cart);
  useEffect(() => {
    handlePrice();
  }, [isCartChanged]);
    ...
    <Router>
      <Routes>
        ...
        <Route
          path="/menu"
          element={
            <ProductMenu handleClick={handleClick} handleClear={handleClear} />
          }
        />
        <Route
          path="/cart"
