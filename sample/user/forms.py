...
def name_validation(value):
        if not re.findall('[a-zA-Z]', value):
            raise ValidationError('Username should contain string only!')
        else:
            return True

def password_validation(value):        
        if not re.findall('[A-Z]', value):
            raise ValidationError('Your password must contain atleast 1 capital letter!')

        elif not len(value) > 9:
            raise ValidationError('Minimum password length is 10!')

        elif not re.findall('[()[\]|\\`~!@#$%^&*_\-+=;:\'",<>./?]', value):
            raise ValidationError('Your  password should contain atleast 1 special character!')
...
class EmployeeCreationForm(forms.Form):
    ...
    def save(self):
        ...
        user_app = UserProfile.objects.create(user_id=user.id, leave_days=self.cleaned_data['leave_days'], gender=self.cleaned_data['gender'], manager_name=self.cleaned_data['manager_name'], group=self.cleaned_data['group'], bio=self.cleaned_data['bio'])
        user_app.save()
...
class ManagerCreationForm(forms.Form):
    ...
    def save(self):
        ...
        user_app = UserProfile.objects.create(user_id=user.id, domain=self.cleaned_data['domain'], gender=self.cleaned_data['gender'], group=self.cleaned_data['group'], bio=self.cleaned_data['bio'])
        user_app.save()
        groups = Group.objects.get(name=self.cleaned_data['group'])
        groups.user_set.add(user)
        groups.save()
